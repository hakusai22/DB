package com.hakusai.db.backend.parser;
...
import com.hakusai.db.backend.parser.statement.Drop;
import com.hakusai.db.backend.parser.statement.Insert;
import com.hakusai.db.backend.parser.statement.Show;
import com.hakusai.db.backend.parser.statement.SingleExpression;
import com.hakusai.db.backend.parser.statement.Update;
import com.hakusai.db.backend.parser.statement.Where;
...
public class Parser {
    ...
    /**
     * Parser 类则直接对外提供了 Parse(byte[] statement) 方法，核心就是一个调用 Tokenizer 类分割 Token，
     * 并根据词法规则包装成具体的 Statement 类并返回。解析过程很简单，仅仅是根据第一个 Token 来区分语句类型，并分别处理，
     * @param statement
     * @return
     * @throws Exception
     */
